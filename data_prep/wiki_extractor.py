...
def parse_wiki_dump(input_path,
                    output_path,
                    json_format=False,
                    html_format=False,
                    ...
                    quiet=False):
    """
    Function used to parse wiki dumps into more managable formats

        :param input_path: Path to XML wiki dump file
        :param output_path: Path to output folder
        :param json_format=False: Write output in json format instead of raw text
        :param html_format=False: Write output in html format instead of raw tet
        :param include_sections=True: Preserve sections
        :param include_lists=True: Preserve lists
        :param include_links=True: Preserve links
        :param include_tables=False: Preserve tables in the output article text 
        :param min_text_lengths=-1: Minimum expanded text length required to write document (-1 = no minimum)
        :param max_file_size="1M": The maximum size of each file created
        :param ignore_tags=[]: List of tags that will be dropped, keeping their content
        :param discard_elements=[] List of elements that will be removed from the article text
        :param num_processes=-1: Number of processes to use (Default: max(1, (num_cpus - 1)))
        :param quiet=False: Whether to log the process or not 
    """
    global options

    options.quiet = quiet
    options.keepLinks = include_links
    options.keepSections = include_sections
    options.keepLists = include_lists
    options.write_json = json_format
    options.keep_tables = include_tables
    options.filter_disambig_pages = filter_disambig_pages,
    options.min_text_length = min_text_length if min_text_length > 0 else options.min_text_length
    options.debug = False
    options.discardElements = discard_elements

    options.toHTML = html_format
    if html_format:
        options.keepLinks = True
    

    # ignore_tags and discardElemets have default values already supplied, if passed in the defaults are overwritten
    if not ignore_tags:
        ignore_tags = [
    ...
    # Deduce how many processes to use
    num_processes = num_processes if num_processes > 0 else max(
        1, (cpu_count() - 1))

    # Deduce maximum filesize

    power = 'kmg'.find(max_file_size.lower()[-1]) + 1
    file_size = int(max_file_size[:-1]) * 1024**power
