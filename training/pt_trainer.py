...
for epoch in range(epochs):
    ...
    eval_data = DataloaderBPE(drive_path=drive_path, embedder=oov_embedder, embedding_method="laser", language="da",
                              batch_size=1, debug=debug, fold=0, eval=True)
    i_eval = 0
    ...
    eval_data = DataloaderBPE(drive_path=drive_path, embedder=oov_embedder, embedding_method="laser", language="da",batch_size=1,debug=debug,fold=1,eval=True)
    i_eval = 0
    test_loss = 0
    test_sentences = []
    for eval_x, eval_y,queries,targets in iter(eval_data):
