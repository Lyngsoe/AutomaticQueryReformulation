...
from models.rl_transformer_full import RLTransformer
...
if load:
    load_path = "/media/jonas/archive/master/data/squad_real/experiments/TransformerBERT__12-23_00:57"
    model = RLTransformer(base_path, reward_function,embeddings=embeddings,num_layers=n_layers, output_size=vocab_size, device=device,nhead=nhead, dropout=dropout, d_model=d_model, dff=dff, lr=lr,l2=l2)
    ...
    model = RLTransformer(base_path, reward_function,embeddings=embeddings, output_size=vocab_size, device=device,nhead=nhead, dropout=dropout, d_model=d_model, dff=dff, lr=lr,l2=l2)
